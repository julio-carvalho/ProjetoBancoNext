			...
			case 8:
				...
				if (!conta.getPix().isAtivado()) {
					...
					while (opcChave < 1 || opcChave > 4) {
						System.out.println(
								"Opção inválida, digite novamente a opção do tipo de chave que você deseja cadastrar: ");
						System.out.println("1 - CPF\n2 - EMAIL\n3 - TELEFONE\n4 - CHAVE ALEATÓRIA");
			...
			case 21:
				...
				if (conta.getCartaoCredito().isAtivo()) {
					...
					if (conta.getCartaoCredito().getApolice().isAtivo()) {
						...
						// valida se a apólice do cartão é referente ao seguro de morte, invalidez ou
						// desemprego
	...
	private static void menuContaPoupanca(ContaPoupanca contaPoup, Conta conta) {
		...
		while (continuar) {
			...
			case 6:
				...
				break;
			case 7:
				System.out.println("\nDesabilitar Conta Poupança");
				System.out.println("Você deseja desabilitar sua conta poupança?\n1 - Sim\n2 - Não");
